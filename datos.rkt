...
(define (get-usuario-autenticado identidad clave)
  (for ([(usuaId casaId usuaIdentidad usuaClave usuaNombres usuaApellido1 usuaApellido2 usuaTelefono) (get-usuario-autenticado-sql identidad clave)])
    (set! usuario-sistema (append (list usuaIdentidad) (list usuaNombres) (list usuaApellido1) (list usuaApellido2) (list usuaTelefono)))))
;--------------------------------------------------------------------------------------------------------------------------------
;retorna una lista con los consumos de un contador en los ultimos 'limite' mes
(define (contador-consumos-sql contId limite)
  (query-list conexion "select consumo from (select year(histFechaC) as anio, month(histFechaC) as mes, histRegistroC as consumo from HISTORIAL_CONTADORES where contId = ? order by anio asc, mes asc limit ?) as CONSUMOS" contId limite))
;--------------------------------------------------------------------------------------------------------------------------------
...
(define (get-usuario-consumos-sql contId)
  (in-query conexion "select usuaId, sum(consRegistroH) as suma from HISTORIAL_CONSUMOS inner join SENSORES on HISTORIAL_CONSUMOS.sensId = SENSORES.sensId where year(consFechaH) = year(curdate()) and month(consFechaH) = month(curdate()) and SENSORES.contId = ? group by usuaId order by suma desc" contId))

(define (get-usuario-consumos-count-sql contId)
  (query-list conexion "select count(*) from (select usuaId as cuenta from HISTORIAL_CONSUMOS inner join SENSORES on HISTORIAL_CONSUMOS.sensId = SENSORES.sensId where year(consFechaH) = year(curdate()) and month(consFechaH) = month(curdate()) and SENSORES.contId = ? group by usuaId) as CONSUMOS" contId))
...
(define (get-usuario-consumo contId tipo cont)
  (if (equal? tipo <)
      ...
      (set! cont 0))
  (for ([(usuaId suma) (get-usuario-consumos-sql contId)]) 
    (if (= i cont) (set! usuario-consumo (cons usuaId suma)) (+ 0 0)) (set! i (+ i 1))) (set! i 0))
;--------------------------------------------------------------------------------------------------------------------------------
